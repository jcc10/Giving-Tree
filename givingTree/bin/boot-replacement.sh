#!/bin/sh
# GivingTree Boot Replacement Script
# Prevents Amazon framework from starting and launches GivingTree instead
# Part of the "Amazon-Free Boot" system

LAUNCHER_DIR="/mnt/us/extensions/givingTree"
LOG_FILE="/var/tmp/givingTree-boot.log"
...
BOOT_COUNT_FILE="/var/tmp/givingtree-boot-count"
BOOT_FAILED_FILE="/var/tmp/givingtree-boot-failed"
...
KEEPER_PID=$!
echo "$KEEPER_PID" > /var/tmp/givingtree-keeper.pid
log_msg "Framework keeper started (PID: $KEEPER_PID)"

# Launch GivingTree

log_msg "Launching GivingTree..."
...
LAUNCHER_SCRIPT="$LAUNCHER_DIR/bin/launcher.sh"
...
export GIVINGTREE_BOOT_MODE=1
log_msg "Set GIVINGTREE_BOOT_MODE=1"

# Initialize state file
echo "RUNNING" > /var/tmp/givingtree-state
log_msg "Initialized state file"

# GivingTree Watchdog (Monitors and Auto-Restarts)

log_msg "Starting GivingTree watchdog..."
...
WATCHDOG_PID=$!
echo "$WATCHDOG_PID" > /var/tmp/givingtree-watchdog.pid
log_msg "Watchdog started (PID: $WATCHDOG_PID)"

# Launch GivingTree (Initial Start)

# Change to launcher directory
cd "$LAUNCHER_DIR" || exit 0

# Launch GivingTree
log_msg "Launching GivingTree (initial start)..."
"$LAUNCHER_SCRIPT" &

log_msg "GivingTree boot sequence complete"
