...
# Installs GivingTree as the default boot UI (replaces Amazon framework)

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LAUNCHER_DIR="$(dirname "$SCRIPT_DIR")"
UPSTART_CONF="/etc/upstart/givingtree-boot.conf"
SOURCE_CONF="$LAUNCHER_DIR/givingtree-boot.conf"
...
$FBINK -y 11 -pm "• Boot to GivingTree by default"
...
if [ ! -f "$SOURCE_CONF" ]; then
    $FBINK -y 23 -pm "ERROR: ERROR: givingtree-boot.conf not found!"
...
$FBINK -y 11 -pm "OK: Next reboot will launch GivingTree"
$FBINK -y 12 -pm ""
...
$FBINK -y 20 -pm "            givingtree-boot.conf"
...
$FBINK -f
sleep 0.3
$FBINK -y 15 -pmh "Rebooting to GivingTree..."
